...
#[test]
fn truncated_lua_binary_payloads_report_truncation() -> Result<(), Box<dyn std::error::Error>> {
    let actual = run_binary_with(RunOptions {
        args: vec![
            "-e".to_owned(),
            "local c = string.dump(function() return 1 end); local f, message = load(string.sub(c, 1, 41)); print(f == nil, string.find(message, 'truncated') ~= nil)".to_owned(),
        ],
        ..RunOptions::default()
    })?;
    let expected = ExpectedRun {
...
#[test]
fn table_insert_appends_and_shifts_values() -> Result<(), Box<dyn std::error::Error>> {
    let actual = run_binary_with(RunOptions {
        args: vec![
            "-e".to_owned(),
            "local t = {1, 3}; table.insert(t, 2, 2); table.insert(t, 4); print(t[1], t[2], t[3], t[4])"
                .to_owned(),
        ],
        ..RunOptions::default()
    })?;
...
#[test]
fn table_unpack_rejects_unbounded_result_ranges() -> Result<(), Box<dyn std::error::Error>> {
    let actual = run_binary_with(RunOptions {
        args: vec![
            "-e".to_owned(),
            "local ok, err = pcall(table.unpack, {}, 0, math.maxinteger); print(ok, err:find('too many results') ~= nil)"
                .to_owned(),
        ],
        ..RunOptions::default()
    })?;
...
#[test]
fn table_move_copies_ranges_with_overlap() -> Result<(), Box<dyn std::error::Error>> {
    let actual = run_binary_with(RunOptions {
        args: vec![
            "-e".to_owned(),
            "local t = {10, 20, 30}; table.move(t, 1, 3, 2); print(t[1], t[2], t[3], t[4])"
                .to_owned(),
        ],
        ..RunOptions::default()
    })?;
    let expected = ExpectedRun {
        stdout: b"10\t10\t20\t30\n",
        stderr: b"",
        exit_code: Some(0),
    };
    compare_run(&actual, &expected)?;
...
#[test]
fn pcall_preserves_non_string_error_objects() -> Result<(), Box<dyn std::error::Error>> {
    let actual = run_binary_with(RunOptions {
        args: vec![
            "-e".to_owned(),
            "local t = {}; local ok, err = pcall(error, t); print(ok, err == t)".to_owned(),
        ],
        ..RunOptions::default()
    })?;
    let expected = ExpectedRun {
