...
| QA gate | blocked | 2026-07-19, binary-header validation slice: `cargo fmt --check`, `cargo test` (78 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. `KUU_SMOKE_TIMEOUT_SECONDS=1 ./scripts/run-upstream-smoke.sh` exits 25 after classifying all 34 scripts; `calls.lua` remains a non-timeout failure in corrupted-header coverage. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
