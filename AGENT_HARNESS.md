...
| Phase 12 full conformance | in progress | 2026-07-19, `9bd950b` plus uncommitted `src/vm.rs`, `tests/vm_phase7.rs`, and smoke cleanup: failing-first regression proves `load` accepts a function reader and treats an empty returned string as EOF; positive `string.sub` starts now return empty beyond the end. The full `calls.lua` reader section still times out under its heavy GC workload. `coroutine.yield` and resumable thread execution remain unimplemented. The hardened runner now TERM-waits and KILL-fallbacks, leaving no Kuu process in the bounded smoke receipt. |
...
| QA gate | blocked | 2026-07-19, `9bd950b` plus uncommitted load/cleanup slice: `cargo fmt --check`, `cargo test` (68 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. `KUU_SMOKE_TIMEOUT_SECONDS=1 ./scripts/run-upstream-smoke.sh` exits 25 after classifying all 34 scripts; `calls.lua` is classified timeout. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
