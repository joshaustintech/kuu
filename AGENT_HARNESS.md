...
| Phase 12 full conformance | in progress | 2026-07-20, GC closure marking, loaded-chunk `debug.getinfo` source, reader error returns, `table.remove`, parser-error wording, non-table `_ENV`, debug upvalue access/mutation/join, loaded-chunk varargs, binary dumped-closure nil-upvalues, Lua-compatible dump headers, binary pack formats, corrupted/truncated header/payload diagnostics, and 254-return limit pass. Full `calls.lua` still has a binary-string mutation assertion beyond focused coverage. `coroutine.yield` and resumable thread execution remain unimplemented. Hardened runner TERM-waits and KILL-fallbacks, leaving no Kuu process in bounded smoke receipt. |
...
| QA gate | blocked | 2026-07-20, return-limit slice: `cargo fmt --check`, `cargo test` (81 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. `KUU_SMOKE_TIMEOUT_SECONDS=1 ./scripts/run-upstream-smoke.sh` exits 25 after classifying all 34 scripts; `calls.lua` remains a non-timeout failure in its broader binary-string mutation loop. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
