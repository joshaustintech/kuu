...
| Phase 12 full conformance | in progress | 2026-07-20, plus binary-safe `gsub`, explicit zero-end `string.sub`, and out-of-range `debug.upvalueid` returning nil. Full `calls.lua` exits 0; `closure.lua` advances to unimplemented `string.gmatch`. `coroutine.yield` and resumable thread execution remain unimplemented. Hardened runner TERM-waits and KILL-fallbacks, leaving no Kuu process in bounded smoke receipt. |
...
| QA gate | blocked | 2026-07-20, closure upvalue-id slice: `cargo fmt --check`, `cargo test` (83 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. Bounded smoke keeps `calls.lua` passing; `closure.lua` now fails at `string.gmatch`. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
