...
| Phase 12 full conformance | in progress | 2026-07-20, plus binary-safe `gsub`, explicit zero-end `string.sub`, out-of-range `debug.upvalueid` nil, and exact-pattern `string.gmatch` iterator support. Full `calls.lua` and `closure.lua` exit 0; next corpus frontier is tracked by smoke. `coroutine.yield` and resumable thread execution remain unimplemented. Hardened runner TERM-waits and KILL-fallbacks, leaving no Kuu process in bounded smoke receipt. |
...
| QA gate | blocked | 2026-07-20, gmatch closure slice: `cargo fmt --check`, `cargo test` (84 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. Bounded smoke now keeps `calls.lua` and `closure.lua` passing. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
