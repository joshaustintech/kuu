...
| Phase 12 full conformance | in progress | 2026-07-19, GC closure marking, loaded-chunk `debug.getinfo` source, reader error returns, and `table.remove` regressions pass. Full `calls.lua` now advances past heavy-GC reader and function metadata sections; next failure is its expected parser-error text assertion. `coroutine.yield` and resumable thread execution remain unimplemented. Hardened runner TERM-waits and KILL-fallbacks, leaving no Kuu process in bounded smoke receipt. |
