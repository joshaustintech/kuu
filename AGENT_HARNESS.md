...
| QA gate | blocked | 2026-07-20, calls conformance slice: `cargo fmt --check`, `cargo test` (82 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. Bounded smoke now has passing `calls.lua`; remaining corpus failures are tracked by script. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
