...
| Phase 12 full conformance | in progress | 2026-07-19, GC closure marking, loaded-chunk `debug.getinfo` source, reader error returns, `table.remove`, parser-error wording, and non-table `_ENV` regressions pass. Full `calls.lua` now advances through load/error/environment coverage; next failure is the unimplemented `debug.getupvalue` API. `coroutine.yield` and resumable thread execution remain unimplemented. Hardened runner TERM-waits and KILL-fallbacks, leaving no Kuu process in bounded smoke receipt. |
...
| QA gate | blocked | 2026-07-19, conformance slice: `cargo fmt --check`, `cargo test` (72 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. `KUU_SMOKE_TIMEOUT_SECONDS=1 ./scripts/run-upstream-smoke.sh` exits 25 after classifying all 34 scripts; `calls.lua` now fails without timeout at parser-error text. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
