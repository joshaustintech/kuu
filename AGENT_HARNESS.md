...
| Phase 12 full conformance | in progress | 2026-07-19, `0d7d653` plus uncommitted `src/vm.rs` and `tests/vm_phase7.rs`: failing-first regression proves `table.sort` mutates arrays using default numeric/string ordering and custom comparators. Bounded smoke remains 9 pass, 21 fail, 4 timeout; `calls.lua` advances through table sorting and now stops at generic function-based `load`. `coroutine.yield` and resumable thread execution remain unimplemented. Each smoke run left no Kuu process. |
...
| QA gate | blocked | 2026-07-19, `0d7d653` plus uncommitted table-sort slice: `cargo fmt --check`, `cargo test` (67 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. `KUU_SMOKE_TIMEOUT_SECONDS=1 ./scripts/run-upstream-smoke.sh` exits 25 after classifying all 34 scripts: 9 pass, 21 fail, 4 timeout. Post-smoke process check found no Kuu process. |
