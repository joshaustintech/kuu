...
| Phase 12 full conformance | in progress | 2026-07-19, `e725000` plus uncommitted `src/vm.rs`, `src/resolve/scope.rs`, and `tests/vm_phase7.rs`: failing-first regressions prove recursive multi-return count propagation and local shadowing of a prior same-block global declaration while preserving nested global-function precedence. Bounded smoke remains 9 pass, 21 fail, 4 timeout; `calls.lua` advances through multiple returns and now stops at unimplemented `table.sort`. `coroutine.yield` and resumable thread execution remain unimplemented. Each smoke run left no Kuu process. |
...
| QA gate | blocked | 2026-07-19, `e725000` plus uncommitted multi-return/scope slice: `cargo fmt --check`, `cargo test` (66 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. `KUU_SMOKE_TIMEOUT_SECONDS=1 ./scripts/run-upstream-smoke.sh` exits 25 after classifying all 34 scripts: 9 pass, 21 fail, 4 timeout. Post-smoke process check found no Kuu process. |
