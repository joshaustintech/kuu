...
| Phase 12 full conformance | in progress | 2026-07-19, `b377c68` plus uncommitted `src/vm.rs` and `tests/vm_phase7.rs`: failing-first fixed-point regression proves a tail call reuses the enclosing frame base, closing captured locals before stack reuse. Bounded smoke remains 9 pass, 21 fail, 4 timeout; `calls.lua` advances through its fixed-point closure and reaches a later non-callable error. `coroutine.yield` and resumable thread execution remain unimplemented. Each smoke run left no Kuu process. |
...
| QA gate | blocked | 2026-07-19, `b377c68` plus uncommitted tail-frame slice: `cargo fmt --check`, `cargo test` (64 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. `KUU_SMOKE_TIMEOUT_SECONDS=1 ./scripts/run-upstream-smoke.sh` exits 25 after classifying all 34 scripts: 9 pass, 21 fail, 4 timeout. Post-smoke process check found no Kuu process. |
