...
| Phase 12 full conformance | in progress | 2026-07-19, GC closure marking fix plus regression: unreachable closure cycles no longer re-enqueue forever during `collectgarbage()`. Full `calls.lua` now advances past its heavy-GC reader section and fails next at the unsupported `debug.getinfo` stack-level contract. `coroutine.yield` and resumable thread execution remain unimplemented. Hardened runner TERM-waits and KILL-fallbacks, leaving no Kuu process in bounded smoke receipt. |
...
| QA gate | blocked | 2026-07-19, GC closure marking slice: `cargo fmt --check`, `cargo test` (69 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. `KUU_SMOKE_TIMEOUT_SECONDS=1 ./scripts/run-upstream-smoke.sh` exits 25 after classifying all 34 scripts; `calls.lua` now fails without timeout. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
