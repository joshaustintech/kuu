...
| Phase 12 full conformance | in progress | 2026-07-20, plus binary-safe `gsub`, explicit zero-end `string.sub`, out-of-range `debug.upvalueid` nil, exact-pattern `string.gmatch`, table preallocation accounting, `table.insert`, bounded `table.unpack`, and overlap-safe `table.move`. `sort.lua` now passes creation/unpack/insert phases but remains a supported failure in later metamethod move assertions. `coroutine.yield` and resumable thread execution remain unimplemented. Hardened runner TERM-waits and KILL-fallbacks, leaving no Kuu process in bounded smoke receipt. |
...
| QA gate | blocked | 2026-07-20, table stdlib slice: `cargo fmt --check`, `cargo test` (88 `vm_phase7` tests), and `cargo clippy --all-targets -- -D warnings` exit 0. Forbidden-pattern scan reports only crate-root `#![forbid(unsafe_code)]`. Bounded smoke keeps `calls.lua` passing; `sort.lua` remains a supported failure in later metamethod move assertions. Post-smoke process check found no Kuu process after TERM/KILL fallback. |
