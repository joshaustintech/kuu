...
| Phase 12 full conformance | in progress | 2026-07-19, `a5107d1` plus uncommitted `src/resolve_impl.rs`: the failing-first local/global shadowing regression now passes while nested global-function precedence remains green. Bounded smoke remains 9 pass, 21 fail, 4 timeout; `calls.lua` advances through multiple returns and now stops at unimplemented `table.sort`. `coroutine.yield` and resumable thread execution remain unimplemented. Each smoke run left no Kuu process. |
